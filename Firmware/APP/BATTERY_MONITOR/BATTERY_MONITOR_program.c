...
/*
 * Relay control enable flag:
 * ENABLE  -> BATMON can drive relay pin normally
 * DISABLE -> BATMON updates measurements only and does not drive relay pin
 */
static u8 BATMON_u8RelayControlEnable = BATMON_RELAY_CTRL_ENABLE;
...
u8 BATMON_u8Init(void)
{
    ...
    BATMON_u8RelayControlEnable = BATMON_RELAY_CTRL_ENABLE;
...
u8 BATMON_u8Update(void)
{
    ...
    if (Local_u8ErrorState == BATMON_OK)
    {
         ...
         * is applied only when relay control is enabled.
         */
        if (BATMON_u8RelayControlEnable == BATMON_RELAY_CTRL_ENABLE)
        {
...
{
    if ((Copy_u8Enable != BATMON_RELAY_CTRL_DISABLE) &&
        (Copy_u8Enable != BATMON_RELAY_CTRL_ENABLE))
    {
        return BATMON_NOK;
    }

    BATMON_u8RelayControlEnable = Copy_u8Enable;
    return BATMON_OK;
}
...
u8 BATMON_u8GetRelayControlEnable(u8 *Copy_pu8Enable)
{
    ...
    *Copy_pu8Enable = BATMON_u8RelayControlEnable;
    return BATMON_OK;
}
